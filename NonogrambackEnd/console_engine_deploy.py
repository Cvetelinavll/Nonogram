...
def create_nonogram(rows, columns, max_sequential_val):
    ...
    # Create hints for rows and columns
    row_hints = []
    col_hints = []
    ...
    #Generate random column values bw 0/1 and add them to the hints same method
    for j in range(columns):
        temp = 0
        hint = []
        for i in range(rows):
    ...
    #     #Global hints list appends the column sequence
    #     col_hints.append(hint) 
